...
function DoktorDetail() {
    ...
    const name = getLocalizedText(doctor, "name");
    const surname = getLocalizedText(doctor, "surName");
    const role = getLocalizedText(doctor, "role");
    const bioText = doctor?.bio?.[0] ? getLocalizedText(doctor.bio[0], "name") : "";
    const experience = doctor?.experience || "—";
    const rate = doctor?.rate || 5;

    const cardsData = doctor?.doctorSertificates?.length
        ? doctor.doctorSertificates.map((img, index) => ({
            id: index + 1,
            image: `${CERT_DOKTOR_URL}${img}`,
    ...
    return (
        <div id={"doktorDetail"}>
            <div className={"container"}>
                <div className="head">
                    <h1>{t("doctorDetail.title")}</h1>
                    <p data-aos="fade-up" data-aos-delay="100">
                        <Link to="/">{t("contact.breadcrumb.home")}</Link>
                        <div className="dot dot1"></div>
                        <Link to="/doctors">{t("doctorDetail.breadcrumbDoctors")}</Link>
                        <div className="dot dot2"></div>
                        <Link to={`/doktor/${doctor?.id}`}>{`${name} ${surname}`}</Link>
                    </p>
                </div>
                <div className={"doktorContainer"}>
                    <div className={"doktor"}>
                        ...
                        <div className={"content"}>
                            ...
                            <div className={"stars"}>
                                {[...Array(rate)].map((_, i) => (
                                    <svg key={i} xmlns="http://www.w3.org/2000/svg" width="18" height="19" viewBox="0 0 18 19" fill="#2F2F2F">
                                        <path d="M8.99975 13.4525L12.1122 15.335C12.6822 15.68 13.3797 15.17 13.2297 14.525L12.4047 10.985L15.1572 8.60001C15.6597 8.16501 15.3897 7.34001 14.7297 7.28751L11.1072 6.98001L9.68975 3.63501C9.43475 3.02751 8.56475 3.02751 8.30975 3.63501L6.89225 6.97251L3.26975 7.28001C2.60975 7.33251 2.33975 8.15751 2.84225 8.59251L5.59475 10.9775L4.76975 14.5175C4.61975 15.1625 5.31725 15.6725 5.88725 15.3275L8.99975 13.4525Z"/>
                                    </svg>
                                ))}
                            </div>
                            <div className={"about"}>
                                ...
                                <div>
                                    ...
                                    <span>{experience} {t("doctorDetail.experienceSuffix")}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className={"doktorDetail"}>
                        <div className={"first-section"}>
                            <h2>{t("doctorDetail.sections.bio")}</h2>
                            <div className={'row'}>
                                <div className={'col-30 col-md-60 col-sm-60 col-xs-60'}>
                                    <div className={"bio"}>
                                        <div><img src={blue}/></div>
                                        <div dangerouslySetInnerHTML={{ __html: bioText || "<p>${t(\"doctorDetail.empty\")}</p>" }} />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div>
                            <div className={'second-section'}>
                                <h2>{t("doctorDetail.sections.certificates")}</h2>

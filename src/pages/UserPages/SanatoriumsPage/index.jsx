...
import { useGetAllSanatoriumQuery } from "../../../services/userApi.jsx";
...
function SanatoriumsPage() {
    ...
    const sanatoriums = getAllSanatorium?.data;
...
const [selectedLocation, setSelectedLocation] = useState("");
    ...
    const localizedLocation = getLocalizedText(item, 'location')?.toLowerCase() || "";
    
    const matchesSearch = localizedName.includes(searchTerm.toLowerCase());
    const matchesLocation = selectedLocation === "" || 
        localizedLocation.includes(selectedLocation.toLowerCase());
    
    return matchesSearch && matchesLocation;
});
const uniqueLocations = [...new Set(
    sanatoriums?.map((item) => getLocalizedText(item, 'location')).filter(Boolean)
)];

    return (
        <div id="sanatoriums-page">
   ...
   <div className={'search'}>
                            <input 
                                ...
                                placeholder={t("adminPanel.sanatoriumPage.search.placeholder")}
                            />
    ...
    <div className="location-filter">
        ...
        >
            <option value="">{t("adminPanel.sanatoriumPage.filter.allLocations")}</option>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" 
            ...
            <circle cx="12" cy="10" r="3"/>
                    ...
                    <div className={'headers'}>
                        <h2>{t("adminPanel.sanatoriumPage.headers.title")}</h2>
                        <p>{t("adminPanel.sanatoriumPage.headers.subtitle")}</p>
                    </div>
                    <div className={'cards'}>
                        <div className={'row'}>
                            {filteredSanatoriums?.length > 0 ? (
                                filteredSanatoriums.map((item, index) => (
                                    <SanatoriumCard key={index} item={item} />
                                ))
                            ) : (
                                <p style={{ textAlign: "center", width: "100%", marginTop: "20px" }}>
                                    {t("adminPanel.sanatoriumPage.noSanatoriumFound")}
                                </p>
                            )}
                        </div>
                    </div>
            ...
            <div className={"bannerAbout"}>
                <img
                    ...
                    alt={t("adminPanel.sanatoriumPage.bannerAlt")}
                />
            </div>
        </div>
    );
